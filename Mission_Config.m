function sys = Mission_Config(preset)
    if nargin<1, preset="APOLLO7_PREFLIGHT_TRIM"; end
    ...
    if any(upper(string(preset))==["HORUS_2B","APOLLO7_PREFLIGHT_TRIM"])
        sys = reference_vehicle.apply(sys,"ARD");
    end
    sys = reference_vehicle.apply(sys,preset);
end
